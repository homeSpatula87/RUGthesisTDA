...
then
  ...
  if [ $stage -le 0 ]; then
    # treat TDA feats as fMLLR feats for rest of training:
    # test
    dir=$data_fmllr/test
    local/nnet/make_fmllr_featsTDA.sh --nj 10 --cmd "$train_cmd" \
       $dir data/test $dir/log $dir/data $model_type || exit 1
    # dev
    dir=$data_fmllr/dev
    local/nnet/make_fmllr_featsTDA.sh --nj 10 --cmd "$train_cmd" \
       $dir data/dev $dir/log $dir/data $model_type || exit 1
    # train
    dir=$data_fmllr/train
    local/nnet/make_fmllr_featsTDA.sh --nj 10 --cmd "$train_cmd" \
       $dir data/train $dir/log $dir/data $model_type || exit 1
    # split the data : 90% train 10% cross-validation (held-out)
    utils/subset_data_dir_tr_cv.sh $dir ${dir}_tr90 ${dir}_cv10 || exit 1
  fi
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/nnet/align.sh --nj 20 --cmd "$decode_cmd" \
